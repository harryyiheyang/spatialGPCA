...
RTriangle internally. The package projects and buffers boundary coordinates;
sf is not required.
...
- `buffer`: fixed outward offset in **km**, 50 for the US and 5 for the UK.
  It is independent of the correlation range. The mesh function applies it to
  `boundary`; with custom `vertices` and `segments`, it records it without applying it again.
...
GP uses rho = 135.853 km; the recorded SPDE run matches exp(-1) at 150 km and used
1,213 mesh vertices with a fixed 50 km buffer. The current sf-free constructor
produces 1,212 vertices for this example; the table retains the recorded timings.
Their covariance models and settings differ. See
